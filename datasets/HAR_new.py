...
class HAR(Dataset):
    ...
    def __getitem__(self, idx):
       ...
       support_x = np.zeros([self.setsz, 3, self.imgw, self.imgh], dtype = "float32")
       query_x = np.zeros([self.querysz, 3, self.imgw, self.imgh], dtype = "float32")

       support_y = np.zeros((self.setsz, 1), dtype = "float32")
       query_y = np.zeros((self.querysz, 1), dtype = "float32")
       ...
       if self.transform:
           for i, spt_x in enumerate(flatten_support_x):
                ...
                support_x[i] = np.tile(self.train_transform(spt_x_img.cast('float32')),[1,3,1,1])
           ...
           for i, qry_x in enumerate(flatten_query_x):
                ...
                query_x[i] = np.tile(self.train_transform(qry_x_img.cast('float32')), [1,3,1,1])
       ...
       return support_x, support_y.squeeze(), query_x, query_y.squeeze()

    def get_finetune_item(self):
        
       finetune_csi = self.ft_csi
       new_csi = np.ones(shape=finetune_csi.shape, dtype = "float32")
       new_csi = np.tile(new_csi.reshape((finetune_csi.shape[0],1,260,200)),[1,3,1,1])
    ...
    def get_trg_item(self):
       ...
       trg_csi = self.ts_csi
       new_csi = np.ones(shape=trg_csi.shape, dtype = "float32")
       new_csi = np.tile(new_csi.reshape((new_csi.shape[0],1,260,200)), [1,3,1,1])
